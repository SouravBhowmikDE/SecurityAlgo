...
public class Aescipher {
  ...
  /* This function returns a hex integer from a look up table stored in a hashmap
   ...
   * @param round is the "round number" of AES that ranges from 0 to 10 in this implementation
      ...
      for (int d = 0 ; d < 4 ; d++ ){  
        ...
        for (int k = 0 ; k < 4 ; k++ ){
                  switch(GaloisMatrix[c][k]){
                    case 0x01:sum=sum^inStateHex[k][d];
                              break;
                    case 0x02:sum= sum^multiply2(inStateHex[k][d]);
                              break;
